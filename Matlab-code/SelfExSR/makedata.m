...
for TestImgIndex =1:960
    A=strcat('F:\Ph.D\进行中的任务\SR\data\valid1\',num2str(TestImgIndex),'.png');
%     A=strcat('I:\test.png');
    A=imread(A);
    A= modcrop(A, scale);
    if ndims(A) == 3
...
% for TestImgIndex =1:160
%     A=strcat('F:\github\Personal-Summarize\Matlab-code\SelfExSR\data\rs\image_SRF_4_lvl\',num2str(TestImgIndex),'.bmp');
%     A=imread(A);
% %     A= modcrop(A, scale);
%     B=strcat('C:\Users\wang\Desktop\闄勫姞瀹為獙\blur\2\',num2str(TestImgIndex),'.png');
%     B=imread(B);
% %     B= modcrop(B, scale);
%     strw = strcat(strcat('.\data\',dataname,'\image_SRF_4\img_'),num2str(TestImgIndex, '%03d'),'_SRF_4_HR.png');
%     imwrite(uint8(A),strw,'png');
